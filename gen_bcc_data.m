...
close all
%% 加载数据源
file = '.\docs\std_scramble_data.xlsx';
sheet = 'tx_scramble_data';
...
raw_bit = ['000000',raw_bit] ;
for ii = 1:length(B);
    A(ii) = rem(str2num(raw_bit(ii)) + str2num(raw_bit(ii+1)) + str2num(raw_bit(ii+3)) + str2num(raw_bit(ii+4)) + str2num(raw_bit(ii+6)),2);
    B(ii) = rem(str2num(raw_bit(ii)) + str2num(raw_bit(ii+3)) + str2num(raw_bit(ii+4)) + str2num(raw_bit(ii+5)) + str2num(raw_bit(ii+6)),2);
end
encode_data = [A,B];
...
encode_data = encode_data(:);
save('.\vars\encoded_data.mat','encode_data');
%% 交织
N_CBPS = 48*4;
N_BPSC = 4;
